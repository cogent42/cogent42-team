...
| `project` | team | "Katha launches H2" |
...
| `ANTHROPIC_API_KEY` | Used by all services that call the Claude Agent SDK |
...
Near-term:
- [ ] Magic-link admin auth via Resend
- [ ] User-facing dashboard (each user sees their own facts, can flip ACLs, see who-read-what)
- [ ] Gmail History API cursor (replace polling with delta sync)
- [ ] More connectors: Google Calendar, Slack, Linear, GitHub
- [ ] Per-user BYO Anthropic / OpenAI keys
- [ ] Helm chart for k8s deployment (single-host compose stays the default)

Longer-term:
- [ ] Multi-tenant mode (single deployment hosting multiple orgs)
- [ ] Row-level security in Postgres for stronger ACL enforcement
- [ ] SCIM / SSO provisioning
- [ ] Onboarding mode — new employee joins and inherits curated org-public knowledge from day 1
